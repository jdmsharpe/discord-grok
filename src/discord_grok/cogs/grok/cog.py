...
def extract_tool_info(response_json: dict[str, Any]) -> ToolInfo:
    ...
    for output_item in response_json.get("output", []):
        for content_part in output_item.get("content", []) if isinstance(output_item, dict) else []:
            for annotation in (
                content_part.get("annotations", []) if isinstance(content_part, dict) else []
            ):
...
class xAIAPI(commands.Cog):
        ...
        for attempt in range(1, MAX_API_ATTEMPTS + 1):
            try:
                async with session.post(url, headers=headers, json=json_payload) as resp:
                    ...
                    should_retry = (
                        resp.status in RETRYABLE_STATUS_CODES and attempt < MAX_API_ATTEMPTS
                    )
                    if should_retry:
    ...
    async def _upload_file_attachment(self, attachment: Attachment) -> str | None:
        ...
        try:
            ...
            self.logger.info("Uploaded file %s as %s", attachment.filename, uploaded.id)
            return uploaded.id
        except Exception as error:
            self.logger.warning("Failed to upload file %s to xAI: %s", attachment.filename, error)
            return None
    ...
    async def _cleanup_conversation_files(self, conversation: Conversation) -> None:
        ...
        for file_id in conversation.file_ids:
            try:
                ...
                self.logger.warning("Failed to delete xAI file %s: %s", file_id, error)
    ...
    async def chat(
        ...
        try:
            ...
            if x_search_allowed_handles:
                handles = [
                    h.strip().lstrip("@") for h in x_search_allowed_handles.split(",") if h.strip()
                ]
                if len(handles) > 10:
                    await ctx.send_followup(
                        embed=Embed(
            ...
            if x_search_excluded_handles:
                handles = [
                    h.strip().lstrip("@") for h in x_search_excluded_handles.split(",") if h.strip()
                ]
                if len(handles) > 10:
                    await ctx.send_followup(
                        embed=Embed(
            ...
            if uploaded_file_ids:
                ...
                for fid in uploaded_file_ids:
                    try:
                        ...
                        self.logger.warning(
                            "Failed to clean up orphaned xAI file %s: %s", fid, cleanup_err
                        )
    ...
    async def video(
        ...
        try:
            ...
            self.logger.info("Starting video generation with grok-imagine-video (mode=%s)", mode)
    ...
    async def tts(
        ...
        try:
            ...
            await ctx.send_followup(embeds=embeds, file=File(data, f"speech.{ext}"))
            self.logger.info("Successfully generated and sent TTS audio")

        except Exception as e:
            description = format_xai_error(e)
            self.logger.error(f"TTS generation failed: {description}", exc_info=True)
            await ctx.send_followup(
                embed=Embed(title="Error", description=description, color=Colour.red())
            )
